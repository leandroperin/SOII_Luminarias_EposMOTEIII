...
// GPTM
    ...
    enum GPTMICR {          // Description                         Type Reset value
        TATO_INT = 1 << 0,  // Timer A time-out interrupt clear      RW 0
        CAM_INT  = 1 << 1,  // Timer A capture match interrupt clear RW 0
        CAE_INT  = 1 << 2,  // Timer A capture event Interrupt clear RW 0
        TAM_INT  = 1 << 4,  // Timer A match interrupt clear         RW 0
        TBTO_INT = 1 << 8,  // Timer B time-out interrupt clear      RW 0
        CBM_INT  = 1 << 9,  // Timer B capture match interrupt clear RW 0
        CBE_INT  = 1 << 10, // Timer B capture event Interrupt clear RW 0
        TBM_INT  = 1 << 11, // Timer B match interrupt clear         RW 0
        WUE_INT  = 1 << 16, // write update error interrupt clear    RW 0
    };
